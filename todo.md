...
Items are numbered so they can be referred to as `#N`. Numbers are stable: a done item is removed and its number is not reused, so gaps are expected. The 2026-09-03 review findings come first because they are the current discussion; the older backlog follows.
...
- [ ] 8. `scripts/init.sh` runs `make init`, `npm install`, and composer immediately in any repo. (A 2026-09-01 decision left this as-is; reopened by this review.)
